...
export function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-40 w-full border-b-[3px] border-wood-900 wood-panel shadow-2xl">
      ...
      {menuOpen && (
        <div className="sm:hidden border-t border-wood-700/50 wood-panel px-4 py-2 flex flex-col font-rye tracking-wider">
          {links.map((link) =>
            link.external ? (
              <a key={link.href} href={link.href} target="_blank" rel="noopener noreferrer"
                onClick={() => setMenuOpen(false)}
                className="text-sm font-special text-cream/50 hover:text-wanted transition-colors py-3 border-b border-wood-700/30 last:border-0">
                {link.label}
              </a>
            ) : (
              <Link key={link.href} href={link.href}
                onClick={() => setMenuOpen(false)}
                className="text-base text-cream/80 hover:text-wanted transition-colors py-3 border-b border-wood-700/30 last:border-0">
                {link.label}
              </Link>
            )
          )}
